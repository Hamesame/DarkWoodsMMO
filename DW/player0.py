...
class Player:
    ...
    def new_from_player(self, player):
        ...
        self.pt_code = player.pt_code
        self.pt_name = player.pt_name
...
class Explorer(Player):
    ...
    def travel_to_plot_msg(self):
        ...
        if self.is_at_forest:
            self.is_travelling = True
            self.travel_time = 40*60
            self.travelling_loc = "plot_msg"
            self.bot.send_message(text="You used the map to the sunflower, you will get there in 40 minutes", chat_id=self.chat_id)
...
class Wizard(Player):
    ...
    def print_self_coms(self):
        ...
        if self.pt_code:
            ...
            s += f"/heal (chat_id) to heal {power} hp from party\n"
    ...
    def sanctuary(self):
        s = "You found an ancient library, there you stuied the books and took a rest. All mana has been recovered."
        self.ap=self.max_ap
        self.bot.send_message(text=s,chat_id=self.chat_id)
